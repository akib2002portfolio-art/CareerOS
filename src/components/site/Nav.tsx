    ...
    >
      ...
      >
        ...
        <nav aria-label="Primary" className="hidden items-center gap-10 md:flex">
          {links.map((l) => (
            <Link
              key={l.to}
              to={l.to}
              className="link-underline text-[1.0625rem] text-muted-foreground transition-colors hover:text-foreground data-[status=active]:text-foreground"
            >
              {l.label}
            </Link>
          ))}
        ...
        <div className="hidden items-center gap-3 md:flex">
          <ActionLink to="/signin" variant="ghost" size="md">
            Sign In
          </ActionLink>
          <ActionLink to="/signup" variant="signal" size="md">
            Get Started
          </ActionLink>
        </div>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            transition={{ duration: reduced ? 0 : 0.5, ease: EASE_SMOOTH }}
