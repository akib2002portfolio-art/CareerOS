...
export function ModelSection() {
  ...
  const lineLength = useTransform(smoothProgress, [0.05, 0.95], [0, 1]);
...
function Stage({
  ...
  return (
    <motion.li className="relative" style={reduced ? {} : { opacity: active }}>
      ...
      <p className="mt-2 max-w-md text-base leading-relaxed text-muted-foreground">{stage.body}</p>
    </motion.li>
  );
}
...
function LoopDiagram({
  ...
  // Springed copy of scroll progress just for this diagram, so the rotation
  // and stroke draw glide with a touch of inertia instead of tracking the
  // scrollbar 1:1 — a small but noticeable premium detail.
  const smooth = useSpring(progress, { stiffness: 260, damping: 38, mass: 0.5 });
  const draw = useTransform(smooth, [0.05, 0.9], [0, 1]);
